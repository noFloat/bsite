<?php
require_once(dirname(__FILE__) . "/Testconn.php");
...
function findAll() {
	 ...
	 */
	$fields = array('vid','file','list','message','color','fontsize','mode','playtime','date');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `tm_comment` $where"); 
	...
	$query_recordset = "SELECT vid,file,list,message,color,fontsize,mode,playtime,date FROM `tm_comment` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `tm_comment` (vid,file,list,message,color,fontsize,mode,playtime,date) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)" ,			GetSQLValueString($_REQUEST["vid"], "int"), # 
			GetSQLValueString($_REQUEST["file"], "text"), # 
			GetSQLValueString($_REQUEST["list"], "text"), # 
			GetSQLValueString($_REQUEST["message"], "text"), # 
			GetSQLValueString($_REQUEST["color"], "text"), # 
			GetSQLValueString($_REQUEST["fontsize"], "text"), # 
			GetSQLValueString($_REQUEST["mode"], "text"), # 
			GetSQLValueString($_REQUEST["playtime"], "text"), # 
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `tm_comment` WHERE vid = %s",
		GetSQLValueString($_REQUEST["vid"], "int")
	);
	$recordset = mysql_query($query_recordset, $conn);
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `tm_comment` SET file = %s,list = %s,message = %s,color = %s,fontsize = %s,mode = %s,playtime = %s,date = %s WHERE vid = %s", 
			GetSQLValueString($_REQUEST["file"], "text"), 
			GetSQLValueString($_REQUEST["list"], "text"), 
			GetSQLValueString($_REQUEST["message"], "text"), 
			GetSQLValueString($_REQUEST["color"], "text"), 
			...
			GetSQLValueString($_REQUEST["playtime"], "text"), 
			GetSQLValueString($_REQUEST["date"], "text"), 
			GetSQLValueString($row_recordset["vid"], "int")
		);
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `tm_comment` WHERE vid = %s", 
			GetSQLValueString($row_recordset["vid"], "int")
		);
